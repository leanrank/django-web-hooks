...
def get_hook_model():
    ...
    if django_apps:
        model_label = (model_label or 'django_web_hooks.Hook').replace('.models.', '.')
        try:
        ...
        if model_label in (None, 'django_web_hooks.Hook'):
...
def distill_model_event(
    ...
    """
    from django_web_hooks.config import get_event_actions_config, HOOK_EVENTS
